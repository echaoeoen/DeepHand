...
def openData(filepath):
    with open(filepath, 'r') as f:
        reader = csv.reader(f)
        return np.asarray(list(reader))
@stopwatch
def full_test():
    # config = tf.ConfigProto()
    # config.gpu_options.allow_growth = True
    sess = tf.Session()
    sda = SDAutoencoder(dims=[6000, 3000, 1500,750,400],
    ...
    sda.pretrain_network(X_TRAIN_PATH, epochs=1000)
    trained_parameters = sda.finetune_parameters(X_TRAIN_PATH, Y_TRAIN_PATH, output_dim=6, epochs=1000)
